...
import ObolusMerchantEscrow from '@/lib/artifacts/contracts/ObolusMerchantEscrow.sol/ObolusMerchantEscrow.json';
import WebhookManager from '@/components/WebhookManager';

// cUSDC on Creditcoin Testnet V2
const DEFAULT_STABLECOIN = "0x58e67dEEEcde20f10eD90B5191f08f39e81B6658";
...
export default function AppDetails() {
    ...
    const fetchLogs = async () => {
        ...
        try {
            const externalProvider = await wallet.getEthereumProvider();
            const provider = new ethers.BrowserProvider(externalProvider);
            const contract = new ethers.Contract(app.escrow_contract, ObolusMerchantEscrow.abi, provider);
    ...
    const handleWithdraw = async () => {
        ...
        try {
            ...
            const contract = new ethers.Contract(app.escrow_contract, ObolusMerchantEscrow.abi, signer);
    ...
    const handleDeployEscrow = async () => {
        ...
        try {
            ...
            const factory = new ethers.ContractFactory(
                ObolusMerchantEscrow.abi,
                ObolusMerchantEscrow.bytecode,
                signer
            );
...
const res = await fetch('https://obolus.link/api/bills/create', {
  method: 'POST',
  headers: {
                        ...
                        {app.escrow_contract ? (
                            <div className="space-y-4">
                                ...
                                <p className="text-[10px] text-white/30 leading-relaxed italic">
                                    Your escrow is active on Creditcoin Testnet. Payments will be routed here.
                                </p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                ...
                                <p className="text-[9px] text-white/20 text-center uppercase tracking-widest font-bold">Gas fee: ~0.02 tCTC</p>
                            </div>
                        )}
                    </section>
                    ...
                    <section className="border border-white/5 rounded-2xl p-6 bg-gradient-to-br from-white/[0.02] to-transparent">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between text-[11px]">
                                ...
                                <span className="text-white/60 font-bold">Creditcoin Testnet</span>
                            </div>
                            <div className="flex justify-between text-[11px]">
                                <span className="text-white/30">Currency</span>
                                <span className="text-white/60 font-bold">USC (cUSDC)</span>
                            </div>
                            <div className="flex justify-between text-[11px]">
                                <span className="text-white/30">Chain ID</span>
                                <span className="text-white/60 font-bold">102036</span>
                            </div>
                        </div>
                    </section>
                </div>
